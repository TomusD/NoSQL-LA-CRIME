...
def generate_upvotes():
    ...
    print(f"Generating upvotes for {num_reports_to_upvote} reports")
    ...
    for report in reports_to_upvote:
        ...
        for _ in range(num_upvotes_for_this_report):
            ...
            if len(officer.upvote_details) >= 1000:
                print(f"Officer {officer._id} already has 1000 upvotes")
                continue
            if any(upvote["DR_NO"] == report.DR_NO for upvote in officer.upvote_details):
                continue
